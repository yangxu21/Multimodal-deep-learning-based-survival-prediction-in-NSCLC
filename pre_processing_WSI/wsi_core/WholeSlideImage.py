...
class WholeSlideImage():
    def __init__(self, path):
        ...
        base_width=4000
        wpercent = (base_width / float(self.wsi.size[0]))
        hsize = int((float(self.wsi.size[1]) * float(wpercent)))
        self.wsi_resize = self.wsi.resize((base_width, hsize), Image.Resampling.LANCZOS)
        self.scale = ((self.wsi.size[0] / self.wsi_resize.size[0]), (self.wsi.size[1] / self.wsi_resize.size[1]))
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        scaled_ref_patch_area = int(ref_patch_size**2 / (self.scale[0] * self.scale[1]))
        ...
        self.contours_tissue = self.scaleContourDim(foreground_contours, self.scale)
        self.holes_tissue = self.scaleHolesDim(hole_contours, self.scale)
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.wsi.size[0], self.wsi.size[1])
        ...
        ref_patch_size = (patch_size*patch_downsample[0], patch_size*patch_downsample[1]) # (256, 256)
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                # normalize the color of the patch
                # try:
                #     patch_PIL_2 = norm_HnE(np.array(patch_PIL))
                #     patch_PIL = Image.fromarray(patch_PIL_2)
                # except:
                #     print("Error in normalizing patch")
                #     continue
                ...
                'downsample': (1,1), 'downsampled_level_dim': tuple(np.array(self.wsi.size)//custom_downsample), 'level_dim': self.wsi.size,
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        if len(results)>0:
            ...
            attr = {'patch_size' :            patch_size, # To be considered...
                    ...
                    'downsample':             (1,1),
                    'downsampled_level_dim' : tuple(np.array(self.wsi.size)),
                    'level_dim':              self.wsi.size,
                    'name':                   self.name,
                    'save_path':              save_path}
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        if top_left is not None and bot_right is not None:
            ...
            region_size = self.wsi_resize.size
            top_left = (0,0)
            bot_right = self.wsi.size
            w, h = region_size
